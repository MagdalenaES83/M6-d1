...
UPDATE public.authors   	set birth_year ='1890'   WHERE   id = 2     RETURNING *



------------------------------Update query to update authors last_name  ------------------------------------------
UPDATE public.authors   	set last_name ='Christie'   WHERE   id = 2     RETURNING *
...
SELECT *  FROM public.authors 	WHERE (EXTRACT(YEAR FROM now()) - birth_year) < 60



----------------------------Select authors which are  older than 40 years old ------------------------------------------
SELECT *  FROM public.authors 	WHERE (EXTRACT(YEAR FROM now()) - birth_year) > 40
...
SELECT * FROM public.authors 	ORDER BY (EXTRACT(YEAR FROM now()) - birth_year) ASC
